...
class ChartLeads extends BaseDoubleChart {
  ...
  onTopExternal(tooltipEl, chart, tooltip, dataI) {
    ...
    if (this.topChart.data.datasets.length > 2) {
      ...
      elPlanComparison.textContent = planComparison
      elFactComparison.textContent = factComparison
    ...
    if (this.moreFinances && this.moreFinances.length) {
      ...
      this.moreFinances.forEach(({ warehouse_short_name, finance_planfact }) => {
        ...
        skladBlock.insertAdjacentHTML('beforeend', `<p>${warehouse_short_name}: ${data.leads_accumulated_fact}</p>`)
      })
    }
  }
  ...
  onBottomExternal(tooltipEl, chart, tooltip, dataI) {
    ...
    if (this.moreFinances && this.moreFinances.length) {
      ...
      this.moreFinances.forEach(({ warehouse_short_name, finance_planfact }) => {
        const data = finance_planfact[dataI]
        skladBlock.insertAdjacentHTML('beforeend', `<p>${warehouse_short_name}: ${data.leads_fact}</p>`)
      })
    }
  }
  ...
  addComparisonData({ finance_planfact }) {
    ...
    if (this.topChart.data.datasets.length > 2) {
      ...
      this.topChart.data.datasets[2].data = finance_planfact.map(obj => obj.leads_accumulated_planned);
      this.topChart.data.datasets[3].data = finance_planfact.map(obj => obj.leads_accumulated_fact);
    } else if (this.topChart.data.datasets.length === 2) {
      // Создаем новые datasets
      this.topChart.data.datasets.push(
        {
          label: 'План (сравнение)',
          data: finance_planfact.map(obj => obj.leads_accumulated_planned),
          borderColor: '#FFA500',
        ...
        },
        {
          ...
          data: finance_planfact.map(obj => obj.leads_accumulated_fact),
          borderColor: '#9333EA',
    ...
    const leadsDataComparison = finance_planfact.map(obj => -obj.leads_fact);
    const colorsComparison = finance_planfact.map(obj =>
      obj.leads_fact >= obj.leads_planned ? '#9333EA' : '#FFA500'
    );

    if (this.bottomChart.data.datasets.length > 2) {
      // Обновляем существующие datasets
      this.bottomChart.data.datasets[2].data = finance_planfact.map(obj => -obj.leads_planned);
      this.bottomChart.data.datasets[3].data = leadsDataComparison;
      ...
      this.bottomChart.data.datasets.push(
        {
          label: 'План (сравнение)',
          type: 'line',
          data: finance_planfact.map(obj => -obj.leads_planned),
          borderColor: '#FFA500',
          color: '#FFA500',
          backgroundColor: 'transparent',
          borderWidth: 1,
