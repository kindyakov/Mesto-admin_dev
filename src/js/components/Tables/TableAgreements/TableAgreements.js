...
class TableAgreements extends Table {
	...
	async download(data, isAll) {
		try {
			...
			let reqData = { active, user_type, warehouse_id: this.app.warehouse.warehouse_id };

			if (isAll) {
				reqData.all_agrs = 1;
			} else {
				reqData = { ...reqData, agrids: data.map(obj => obj.agrid), all_agrs: 0 };
			}
