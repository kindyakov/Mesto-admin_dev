...
class BaseTableForecast extends Table {
	constructor(selector, options, params) {
		...
		this.onReadyFunctions.push(
			function (context) {
				[this.select] = Array.from(context.selects.selects).filter(
					select => select.name === 'month_or_day'
				);
		...
		this.gridOptions.columnDefs.forEach(coll =>
			coll.field ? (this.emptyRow[coll.field] = 0) : ''
		);
	...
	toggleEditMode(params) {
		...
		if (!isEditMode) {
			...
			const queryParams = {
				day: data.data,
				month_or_day: this.queryParams.month_or_day || 'day',
			...
			this.setPlan(this.endpoint, queryParams, params).then(data => {
	...
	addEmptyRow() {
		...
		emptyRow.data = dateFormatter(
			new Date(),
			this.select.value === 'day' ? 'yyyy-MM-dd' : 'yyyy-MM'
		);
		emptyRow.isNew = true;
