...
export class Accordion {
  ...
  init() {
    ...
    if (isSuccess) {
      this.accords.forEach(accord => {
        accord.classList.remove('_open')
        if (accord.getAttribute('data-accordion-open') == '') {
          this.open(accord)
        }
      })
      this.events()
  ...
  open(accordion) {
    this.options.isAccordion && this.close(accordion)
  ...
  close(_accordion) {
    this.accords.forEach(accordion => {
      ...
      if (!this.options.isAccordion && _accordion !== accordion) return
