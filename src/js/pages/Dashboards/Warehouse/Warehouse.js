...
class Warehouse extends Dashboards {
	...
	init() {
		...
		if (num_of_floors >= 2) {
			this.selectWarehouseFloors = new Select({
				uniqueName: 'select-warehouse-floors',
	...
	async getData(queryParams = {}) {
		...
		return Promise.all([getRooms(queryParams), getScheme(warehouse_id, floor)]);
	}
	...
	async render() {
		try {
			...
			const [dataDashboard, { finance_planfact = [] }, [dataRooms, scheme]] = await Promise.all([
				getDashboardWarehouse(this.queryParams),
				getFinancePlan({ warehouse_id: this.app.warehouse.warehouse_id }),
